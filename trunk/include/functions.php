...
function xmspotlight_news()
{
	...
	echo "</tr>";
  	...
  	$imgspotlight = array();//array for img spotlight array

	for($counterz = 0; $counterz <= (count($txt) - 2); $counterz ++)
	{
	    $sql= "SELECT imgspotlight FROM ".$xoopsDB->prefix('stories')." WHERE storyid = ".intval($txt[$counterz]);
  ...
  //end
  	...
  	while ($bxadd <= $XMSNEWSAMOUNT)
  	{
	  	...
	  	$snumber=$bxadd + 1;
	  	$sql = "SELECT * FROM ".$xoopsDB->prefix('xmspotlight')." WHERE  xmspotlight_id = ".intval($snumber);
